...
\newline
Wenn \source{sequence[current\_step].tone\_length = full} erreicht ist, wird die Tonlänge beim nächsten Tastendruck wieder auf \source{pause} gesetzt.
...
Diese Funktion ist dafür verantwortlich, dass die Sequenz im Abspielmodus kontinuierlich abgespielt wird. Abhängig von der Tonlänge wird ein Ton mit der Tonhöhe des aktuellen Schritts abgespielt. Die übergebene Tonlänge wird in Hundertstelsekunden übergeben. Da diese abgängig vom Tempo ist, muss sie hier berechnet werden. Der Faktor ergibt sich daraus, dass das Tempo in BPM (engl. beats per minute; \textsl{Schläge pro Minute}) angegeben ist. Ein Ton mit der Länge \source{full} dauert also \(\frac{1}{\source{tempo}}\cdot60\) Sekunden. Wenn man das mit 100 multipliziert, kommt man auf das Ergebnis in Hundertstelsekunden. Die anderen Tonlängen sind lediglich mit 0.25, 0.5 und 0.75 multipliziert.
\[
Tonl\ddot{a}nge~\source{full}~in~Hundertstelsekunden=\frac{1}{\source{tempo}}\cdot60\cdot100=\frac{6000}{\source{tempo}}
\]
\newline
Die LED-Matrix wird wieder aktualisiert und die Schrittnummer wird inkrementiert, wenn der letzte Schritt noch nicht erreicht ist. Falls doch, wird \source{current\_step} wieder nullgesetzt, sodass die Sequenz wieder von vorne beginnt.
